...
const Onepoint = () => {
    ...
    const setdb = () => {
        api.getMethod().then(res => {callfn(res.data.data[2].function, res.data.data[2].variable1); setfn(res.data.data[2].function);  })
    }
    const setequation = () => {
        callfn(fn, xold)
    ...
    const callfn = (fn, xold) => {
        var temp_xold, xnew = xold, e = 1.0, es = 0.000001
        var i = iteration
