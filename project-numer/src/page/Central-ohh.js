...
const Central_ohh = () => {
    ...
    const callfn = (fn, x, h) => {
        ...
        var ans = 0
        if (n == 1) {
            ans += ((math.parse(fn).evaluate({ x: (x + h) }) - (math.parse(fn).evaluate({ x: (x-h) }))) / (2*h))
        } else if (n == 2) {
            ans += (math.parse(fn).evaluate({ x: (x + h ) }))
            ...
            ans += 6 * (math.parse(fn).evaluate({ x: (x) }))
            ans -= 4 * (math.parse(fn).evaluate({ x: (x - h) }))
            ans += (math.parse(fn).evaluate({ x: (x-h*2) }))
            ans /= math.pow(h, 4)
        }
