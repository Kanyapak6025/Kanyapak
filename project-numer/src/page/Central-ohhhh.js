...
const Central_ohhhh = () => {
    ...
    const callfn = (fn, x, h) => {
        ...
        if (n == 1) {
            ...
            ans += 8 * (math.parse(fn).evaluate({ x: (x + h) }))
            ans -= 8 * (math.parse(fn).evaluate({ x: (x - h) }))
            ans += (math.parse(fn).evaluate({ x: x - h * 2 }))
            ans /= (h*12)
        } else if (n == 2) {
            ans -= (math.parse(fn).evaluate({ x: (x + h * 2) }))
            ans += 16 * (math.parse(fn).evaluate({ x: (x + h) }))
            ans -= 30 * (math.parse(fn).evaluate({ x: x }))
            ans += 16 * (math.parse(fn).evaluate({ x: (x - h) }))
            ans -= (math.parse(fn).evaluate({ x: x - h * 2 }))
            ans /= (12 * math.pow(h, 2))
        } else if (n == 3) {
            ans -= (math.parse(fn).evaluate({ x: (x + h * 3) }))
            ans += 8*(math.parse(fn).evaluate({ x: (x + h * 2) }))
            ans -= 13 * (math.parse(fn).evaluate({ x: (x + h) }))
            ans += 13 * (math.parse(fn).evaluate({ x: (x - h) }))
            ans -= 8*(math.parse(fn).evaluate({ x: x - h * 2 }))
            ans += (math.parse(fn).evaluate({ x: (x - h * 3) }))
            ans /= (8 * math.pow(h, 3))
            ...
            ans -= (math.parse(fn).evaluate({ x: (x + h * 3) }))
            ans += 12*(math.parse(fn).evaluate({ x: (x + h * 2) }))
            ans -= 39 * (math.parse(fn).evaluate({ x: (x + h) }))
            ans += 56 * (math.parse(fn).evaluate({ x: x }))
            ans -= 39 * (math.parse(fn).evaluate({ x: (x - h) }))
            ans += 12*(math.parse(fn).evaluate({ x: x - h * 2 }))
            ans -= (math.parse(fn).evaluate({ x: (x - h * 3) }))
            ans /= (6 * math.pow(h, 4))
