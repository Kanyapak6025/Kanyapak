...
const Backward_ohh = () => {
    ...
    const setdb = () => {
        api.getDiff().then(res => {
            n = res.data.data[1].n;
            setx(res.data.data[1].x);
            seth(res.data.data[1].h);
            setfn(res.data.data[1].fx);
            callfn(res.data.data[1].fx, res.data.data[1].x, res.data.data[1].h);
    ...
    const callfn = (fn, x, h) => {
        ...
        var ans = 0
        if (n == 1) {
            ans += 3 * (math.parse(fn).evaluate({ x: x }))
            ans -= 4 * (math.parse(fn).evaluate({ x: (x - h) }))
            ans += (math.parse(fn).evaluate({ x: (x - h * 2) }))
            ans /= (2 * h)
        } else if (n == 2) {
            ans += 2 * (math.parse(fn).evaluate({ x: x }))
            ans -= 5 * (math.parse(fn).evaluate({ x: (x - h) }))
            ans += 4 * (math.parse(fn).evaluate({ x: (x - h * 2) }))
            ans -= (math.parse(fn).evaluate({ x: (x - h * 3) }))
            ans /= math.pow(h, 2)
        } else if (n == 3) {
            ans += 5 * (math.parse(fn).evaluate({ x: x }))
            ans -= 18 * (math.parse(fn).evaluate({ x: (x - h) }))
            ans += 24 * (math.parse(fn).evaluate({ x: (x - h * 2) }))
            ans -= 14 * (math.parse(fn).evaluate({ x: (x - h * 3) }))
            ans += 3 * (math.parse(fn).evaluate({ x: (x - h * 4) }))
            ans /= (2 * math.pow(h, 3))
        } else if (n == 4) {
            ans += 3 * (math.parse(fn).evaluate({ x: x }))
            ans -= 14 * (math.parse(fn).evaluate({ x: (x - h) }))
            ans += 26 * (math.parse(fn).evaluate({ x: (x - h * 2) }))
            ans -= 24 * (math.parse(fn).evaluate({ x: (x - h * 3) }))
            ans += 11 * (math.parse(fn).evaluate({ x: (x - h * 4) }))
            ans -= 2 * (math.parse(fn).evaluate({ x: (x - h * 5) }))
            ans /= math.pow(h, 4)
        }
        console.log(ans)
        setData({ fn: fn })
